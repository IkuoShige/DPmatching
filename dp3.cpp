...
#define DATA_FILE "./output/data.txt"
#define SCRIPT_FILE "./output/script.gp"
...
void generateScriptFile() {
    ...
    if (scriptFile.is_open()) {
        ...
        scriptFile << "set output \"plot.png\"" << endl;
...
int main() {
    ...
    for (h0 = 0; h0 < NUMOFFILE; h0++) {
        ...
        for (h = 0; h < NUMOFFILE; h++) {
            ...
            // std::cout << city_miti.flame << std::endl;
            ...
            for (i = 1; i < city_temp.flame; i++) {
                for (j = 1; j < city_miti.flame; j++) {
                    ...
                    double b = g[i - 1][j - 1] + sqrt(2.0) * d[i][j];
    ...
    system("gnuplot ./output/script.gp");
